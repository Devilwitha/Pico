...
| `build_apk.ps1` | Baut die Android-APK unter Windows (per Docker) |
...
Auf der Steuerungsseite kann jede beliebige Datei hochgeladen werden - sie
wird unter ihrem eigenen Namen gespeichert. Existiert bereits eine
gleichnamige Datei auf dem Pico, wird nur diese ersetzt (die alte Version
bleibt als `<name>.bak` erhalten); andernfalls wird die Datei neu angelegt.
`.py`-Dateien werden vor der Uebernahme per `compile()` auf gueltiges Python
geprueft und starten den Pico danach neu, alle anderen Dateien (z.B. `.html`)
werden sofort ohne Neustart uebernommen.
...
neu anlegen und loeschen. Bearbeiten/Anlegen laeuft ueber denselben
